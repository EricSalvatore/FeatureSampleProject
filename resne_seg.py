...
class SegmentationLightningModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # print(f"pred_masks shape: {pred_masks.shape}, values: {pred_masks}")
        # print(f"targets shape: {targets.shape}, values: {targets}")

        # 计算 IoU
        intersection = (pred_masks == targets).float().sum((1, 2))  # 每个样本的交集
        # pred_masks_nonzero = (pred_masks != 0).float().sum((1, 2))
        # targets_nonzero = (targets != 0).float().sum((1, 2))
        # union = pred_masks_nonzero + targets_nonzero - intersection
        union = ((pred_masks != 0) | (targets != 0)).float().sum((1, 2))  # 直接使用逻辑或
        # print(f"Intersection: {intersection}, Union: {union}")
        # if (intersection < 0).any() or (union < 0).any():
        #     print("Warning: Negative values detected in intersection or union")
        ...
        # 确保iou没有负值
        if torch.any(iou < 0):
            print("Warning: IoU has negative values")
...
def train(args, _train_loader, _val_loader):
    # 训练模型
    # 使用restnet18
    pretrained_model = models.resnet18(pretrained=True)
    model = SegmentationLightningModel(num_classes=args.num_classes, _model_features=pretrained_model)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=r'./seg_res_checkpoint',  # 保存路径
        filename=f'best-{args.model_name}-checkpoint-epoch-{args.pretrained_epoch}-best-acc-'+'{avg_acc:.3f}-curepoch-{epoch}',  # 文件名模板
        save_top_k=1,  # 保存验证集最优的 k 个模型
        mode='max'  # 当 'val_loss' 越小越好时为 'min', 否则为 'max'
    )

    trainer = Trainer(max_epochs=args.pretrained_epoch, devices=[1], accelerator='gpu', callbacks=[checkpoint_callback])
    # todo:长尾数据开关
